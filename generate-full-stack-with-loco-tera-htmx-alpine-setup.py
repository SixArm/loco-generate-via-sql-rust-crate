...
def render_generate_sh(form_slug: str, tables: list) -> str:
    """Render setup.sh with every scaffold call chained into one && command."""
...
def render_index_md(form_slug: str, tables: list) -> str:
    lines = [
        f"# {form_slug} — full-stack-with-loco-htmx-alpine-setup",
        "",
        ...
        "## Files",
        "",
        "- `full-stack-with-loco-htmx-alpine-setup` — one compound shell command (scaffolds",
    ...
    lines += [
        ...
        "```",
        "",
        "See [`AGENTS/full-stack-with-loco-tera-htmx-alpine-setup.md`](../../../AGENTS/full-stack-with-loco-tera-htmx-alpine-setup.md)",
...
def process_form(form_dir: Path) -> int:
    ...
    write_file(
        form_dir / "full-stack-with-loco-tera-htmx-alpine-setup",
